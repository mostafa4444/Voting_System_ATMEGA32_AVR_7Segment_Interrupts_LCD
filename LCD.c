...
void LCD_COMMAND(uint8 cmd){
	...
	Send_Command = (PORTC_Reg & 0x0F) | (cmd & 0xF0); //to send high nipple
	...
	Send_Command = (PORTC_Reg & 0x0F) | (cmd<<4);//to send the Low nipple
	DIO_WritePort(LCD_Data_Port,Send_Command);
	DIO_WriteChannel(LCD_RS,STD_Low);//set RS to zero
	DIO_WriteChannel(LCD_E,STD_Higt);
	_delay_us(20);
...
void LCD_Char(uint8 data){
	...
	uint8 Send_Command = 0;
	Send_Command = (PORTC_Reg & 0x0F) | (data & 0xF0); //to send high nipple
	DIO_WritePort(LCD_Data_Port,Send_Command);
	...
	Send_Command = (PORTC_Reg & 0x0F) | (data<<4);//to send the Low nipple
	DIO_WritePort(LCD_Data_Port,Send_Command);
	DIO_WriteChannel(LCD_RS,STD_Higt);//set RS to zero
	DIO_WriteChannel(LCD_E,STD_Higt);
	_delay_us(20);
